          ...
          {columns.map((column) => (
            <div key={column.name} className="p-4 border rounded-lg">
              ...
              <div className="space-y-1 text-xs text-slate-600">
                <p>
                  Type: <span className="font-medium">{column.type}</span>
                </p>
                <p>
                  Unique:{" "}
                ...
                {column.stats && (
                  <div className="mt-2 pt-2 border-t">
                    {column.stats.mean && (
                      <p>Mean: {column.stats.mean.toFixed(2)}</p>
                    )}
                    {column.stats.min && (
                      <p>Min: {column.stats.min.toFixed(2)}</p>
                    )}
                    {column.stats.max && (
                      <p>Max: {column.stats.max.toFixed(2)}</p>
                    )}
                  </div>
                )}
              </div>
