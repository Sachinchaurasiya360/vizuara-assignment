...
export const PipelineNode = memo(({ data, isConnectable }: PipelineNodeProps) => {
  const Icon = ICONS[data.type];
  const statusStyle = STATUS_STYLES[data.status];
  const StatusIcon = statusStyle.icon;

  return (
    <div className={`relative px-6 py-4 rounded-lg border-2 shadow-sm min-w-[200px] ${statusStyle.border} ${statusStyle.bg}`}>
      {/* Input Handle */}
      {data.type !== 'upload' && (
        <Handle
          type="target"
